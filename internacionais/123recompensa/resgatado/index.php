        ...
        <section id="section_conteudo">
                <div id="resgate_topo">
                    <div id="div_resgate">
                        ...
                        <?php if (isset($_SESSION['Cod_Cliente'])):?>
                            <p id="sub_tit">Brindes resgatados</p>
                        </div>
            ...
            <section id="section_produtos">
                <?php
                    $sqlID = "SELECT Cod_resgate, fk_Cliente_Cod_Cliente, fk_Brinde_Cod_produto, Quantidade FROM resgata viaja WHERE fk_Cliente_Cod_Cliente = {$_SESSION['Cod_Cliente']}";
                    $resultID = $conn->query($sqlID);
                    if ($resultID->num_rows > 0) {
                        while ($rowID = $resultID->fetch_assoc()) {
                            $sqlD = "SELECT Cod_Produto, Nome, Descricao, Preco FROM Brinde WHERE Cod_Produto = {$rowID['fk_Brinde_Cod_produto']}";
                            $resultD = $conn->query($sqlD);
                            if ($resultD->num_rows > 0) {
                                while ($rowD = $resultD->fetch_assoc()) {
                        ?>
                        <div class="produto_individual">
                            <div id="imagem_produto"><img src="../imagens/brinde<?php echo $rowD['Cod_Produto']?>.jpg" alt="brinde <?php echo $rowD['Nome'] ?>"></div>

                            <div id="textos_produto_individual">
                                <div class="nome_desc"><p id="nome_produto"><?php echo $rowD['Nome'] ?></p>
                                <div id="descricao_produto">
                                </div></div>
                                <div id="valor_e_botao">
                                    <p>Quantidade: <?php echo $rowID['Quantidade'] ?></p>
                                </div>
                            </div>
                        </div>
                        <?php
